...
import { Cart, CartOverview, QuantityDirection } from '../../../models';
...
export class ShoppingCartContainer {
  ...
  public productList: Signal<Product[]> = toSignal(
  ...
  public totalCartPrice: Signal<number> = computed(() => {
    const priceList = this.productsCart().map(
      (item: CartOverview) => item.totalPrice
    );
  ...
  changeQuantity({
    ...
    const selectCartProduct: Cart = <Cart>(
      this.cartList().find((item: Cart) => item.productId === id)
    );

    this.cartList.update((value: Cart[]) =>
      value.map((item: Cart) =>
        item.productId === id
          ? {
              ...item,
              productQuantity:
                direction === QuantityDirection.INCREASE
                  ? ++selectCartProduct.productQuantity
                  : --selectCartProduct.productQuantity,
            }
          : item
      )
  ...
  removeFromCart(id: number) {
    const selectProduct: Product = <Product>(
      this.productList().find((item: Product) => item.id === id)
    );
